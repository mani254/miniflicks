...
function Header({ citiesData }) {
	...
	useEffect(() => {
		function handleClick(e) {
			const isNavLinkClicked = navLinksRef.current.some((link) => link && link.contains(e.target));
			const clickedOutside = mobileNavRef.current && !mobileNavRef.current.contains(e.target);
			const hamburgerClicked = hamburgerRef.current && hamburgerRef.current.contains(e.target);

			if (hamburgerClicked) {
				return;
			}

			if (isNavLinkClicked || clickedOutside) {
	...
	return (
		<header className="bg-white/60 backdrop-blur-md border-b border-white/10 shadow-md sticky top-0 z-50">
			<div className="flex items-center max-w-[1400px] px-2 py-1 m-auto justify-between relative">
				<NavLink to="/">
					<div className="flex items-center gap-2 ">
						<div className="logo bg-dark w-11 h-11 rounded-full flex items-center justify-center p-1">
							<img src={mfLogo} alt="miniflics logo" className="w-12"></img>
						</div>

						<h3 className="font-jokerman text-dark">Miniflicks</h3>
					</div>
				</NavLink>

				<nav className="hidden lg:flex items-center gap-10 anm-link">
					<NavLink to="/" className={({ isActive }) => (isActive ? "hover:text-zinc-600 active" : "hover:text-zinc-600")}>
						Home
					</NavLink>
					<NavLink to="/about" className={({ isActive }) => (isActive ? "hover:text-zinc-600 active" : "hover:text-zinc-600")}>
						About
					</NavLink>
					<NavLink to="/gallery" className={({ isActive }) => (isActive ? "hover:text-zinc-600 active" : "hover:text-zinc-600")}>
						Gallery
					</NavLink>
					<NavLink to="/testimonials" className={({ isActive }) => (isActive ? "hover:text-zinc-600 active" : "hover:text-zinc-600")}>
						Testimonials
					</NavLink>
					<NavLink to="/contact" className={({ isActive }) => (isActive ? "hover:text-zinc-600 active" : "hover:text-zinc-600")}>
						Contact
					</NavLink>
				</nav>

				<nav ref={mobileNavRef} className={`fixed flex flex-col gap-6 top-0 ${isMobileNav ? "left-0" : "-left-[100%]"} bg-bright  w-full max-w-[280px] h-screen py-[10px] px-2 transition-all ease-in-out duration-300`}>
					<div className="flex items-center gap-2 ">
						<div className="logo bg-dark w-11 h-11 rounded-full flex items-center justify-center p-1">
							<img src={mfLogo} alt="miniflics logo" className="w-12"></img>
						</div>
					...
					<div className="book-now-btn">
						<NavLink to="/booking/locations">
							<button className="btn-3 w-full">
								<svg fill="#3f72c6" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 512.00 512.00" xmlSpace="preserve" stroke="white" strokeWidth="10px">
									<g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
									<g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
									<g id="SVGRepo_iconCarrier">
										<g>
											<g>
												<path d="M378.473,104.189l-28.348-19.626c-3.791-2.623-8.991-1.678-11.616,2.112c-2.625,3.791-1.678,8.991,2.112,11.616 l28.348,19.626c3.754,2.599,9.018,1.638,11.615-2.111C383.208,112.013,382.263,106.813,378.473,104.189z"></path>
											</g>
										</g>
										<g>
											<g>
												<path d="M435.17,143.439l-28.348-19.625c-3.791-2.625-8.99-1.679-11.616,2.111c-2.626,3.791-1.681,8.991,2.11,11.616 c0,0,28.292,19.586,28.348,19.625c3.611,2.5,8.978,1.699,11.616-2.111C439.905,151.265,438.96,146.064,435.17,143.439z"></path>
											</g>
										</g>
										<g>
											<g>
												<path d="M167.004,134.473l-33.566-7.877c-4.487-1.052-8.98,1.731-10.035,6.22c-1.053,4.489,1.732,8.981,6.22,10.035l33.566,7.877 c0.642,0.15,1.282,0.223,1.916,0.223c3.787,0,7.216-2.597,8.12-6.443C174.277,140.018,171.492,135.525,167.004,134.473z"></path>
											</g>
										</g>
										<g>
											<g>
												<path d="M234.138,150.227l-33.566-7.877c-4.488-1.052-8.98,1.731-10.035,6.22c-1.053,4.489,1.732,8.981,6.22,10.034l33.566,7.877 c0.642,0.15,1.282,0.223,1.916,0.223c3.787,0,7.216-2.596,8.12-6.442C241.411,155.772,238.626,151.28,234.138,150.227z"></path>
											</g>
										</g>
										<g>
											<g>
												<path d="M247.23,189.779l-76.055-17.847c-4.487-1.052-8.98,1.731-10.035,6.22c-1.053,4.489,1.732,8.981,6.22,10.034l67.928,15.94 l-54.804,233.553L53.627,407.911l54.803-233.553l25.824,6.059c4.489,1.053,8.98-1.732,10.034-6.22 c1.054-4.488-1.732-8.981-6.221-10.034l-33.95-7.966c-4.478-1.052-8.979,1.731-10.034,6.22L35.465,412.224 c-1.053,4.489,1.732,8.981,6.22,10.034l143.11,33.582c0.63,0.148,1.271,0.22,1.908,0.22c1.542,0,3.066-0.427,4.398-1.252 c1.882-1.167,3.223-3.033,3.73-5.188l58.619-249.807C254.502,195.324,251.717,190.832,247.23,189.779z"></path>
											</g>
										</g>
										<g>
											<g>
				...
				<div className="hamburger-wrapper block lg:hidden" ref={hamburgerRef} onClick={(e) => e.stopPropagation()}>
					<label className="hamburger">
						<input type="checkbox" checked={isMobileNav} onChange={() => setIsMobileNav(!isMobileNav)} />
						<svg viewBox="0 0 32 32">
							<path className={`line line-top-bottom ${isMobileNav ? "checked" : ""}`} d="M27 10L13 10C10.8 10 9 8.2 9 6C9 3.5 10.8 2 13 2C15.2 2 17 3.8 17 6L17 26C17 28.2 18.8 30 21 30C23.2 30 25 28.2 25 26C25 23.8 23.2 22 21 22L7 22"></path>
