  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <div className="text-center">
        <p className="text-sm font-medium text-gray-700">No add-ons yet</p>
        <p className="text-xs text-gray-400 mt-1">
          Add items like LED Name, Smoke Entry, Rose Petals.
        </p>
      </div>
...
function Addons() {
  ...
  const { admin } = useAuth();
  const { addonData, refetch } = useOutletContext() || {};

  const addons = addonData?.addons || [];
  const loading = addonData?.loading;
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      await catalogApi.deleteAddon(selected._id);
  ...
  async function handleStatusChange(addon) {
    setTogglingId(addon._id);
    const updatedStatus = !addon.status;
    try {
      await catalogApi.updateAddon(addon._id, {
        ...addon,
        status: updatedStatus,
      });
      toast.success(`Add-on ${updatedStatus ? "activated" : "deactivated"}`);
      refetch?.();
    } catch (err) {
  ...
  const totalActive = addons.filter((a) => a.status !== false).length;
  const totalInactive = addons.length - totalActive;
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Manage extra experiences and party props available to bookers.
            </p>
          </div>
          <Button onClick={() => navigate("/admin/addons/add")}>
        ...
        <Card>
          {loading ? (
            ...
            <EmptyState onAdd={() => navigate("/admin/addons/add")} />
          ) : (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {togglingId === addon._id ? (
                            <Loader2 className="w-4 h-4 animate-spin text-blue-600" />
                          ) : (
                            <Switch
                              checked={addon.status !== false}
                              onCheckedChange={() => handleStatusChange(addon)}
                            />
                          )}
                          <span
                            className={`text-xs font-medium ${addon.status !== false ? "text-emerald-600" : "text-gray-400"}`}
                          >
                            {addon.status !== false ? "Active" : "Inactive"}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <Button
                            ...
                            onClick={() =>
                              navigate(`/admin/addons/edit/${addon._id}`)
                            }
                            title="Edit"
                          >
