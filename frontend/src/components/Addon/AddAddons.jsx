  ...
  return (
    <div className="space-y-1.5">
      <label className="text-sm font-medium text-gray-700">
        Add-on Image <span className="text-red-500">*</span>
      </label>
...
/* ─── Main Component ────────────────────────────────────────────────── */
function AddAddon({ update = false }) {
  ...
  useEffect(() => {
    if (update && addonData?.addons?.length > 0) {
      const current = addonData.addons.find((a) => a._id === id);
      if (!current) return;
      setDetails({
        name: current.name || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (update) {
        await catalogApi.updateAddon(id, formData);
        toast.success("Add-on updated successfully");
      } else {
        await catalogApi.addAddon(formData);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900">
            {update ? "Update Add-on" : "Add Add-on"}
          </h1>
          <p className="text-xs text-gray-500 mt-0.5">
            {update ? "Modify existing experience or prop details" : "Add a party prop or booking experience add-on"}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Configure name, unit price, and image.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. LED Name, Smoke Entry, Rose Petals on Table"
                required
            ...
            <div className="flex items-center justify-between p-3.5 rounded-lg border border-gray-200 bg-gray-50/50">
              <div>
                <p className="text-sm font-medium text-gray-800">Add-on Status</p>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button type="submit" disabled={submitting}>
            ...
            {submitting ? "Saving…" : update ? "Update Add-on" : "Save Add-on"}
          </Button>
        </div>
      </form>
    </div>
