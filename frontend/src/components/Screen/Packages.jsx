...
const PackageForm = ({ packages, setPackages }) => {
  ...
  const handleAddonChange = (index, addon) => {
    ...
    if (newPackages[index].addons.includes(addon)) {
      newPackages[index].addons = newPackages[index].addons.filter(
        (a) => a !== addon
      );
    } else {
  ...
  return (
    <div className="outer-box">
      ...
      {packages.map((pkg, index) => (
        <div
          key={index}
          className="border border-dashed border-gray-400 p-5 rounded-md mb-4 relative"
        >
          ...
          <div className="flex flex-wrap">
            {availableAddons.map((addon) => (
              <div
                key={addon}
                className="input-wrapper checkbox flex gap-2 items-center w-1/3"
              >
                <input
                  type="checkbox"
                  checked={pkg.addons.includes(addon)}
                  onChange={() => handleAddonChange(index, addon)}
                />
                <label
                  htmlFor={addon}
                  className="whitespace-nowrap w-full  peer-checked:text-logo"
                >
                  {addon}
                </label>
              </div>
          ...
          <span
            className="cursor-pointer text-2xl flex items-center p-1 absolute top-1 right-1 bg-gray-100 rounded-full"
